...
Allow lateral on brake with panda disallowing long messages on brake, still working on bap allowing the use of the car lane assist activation menu, no car Alert for steering unavailable on start stop Motor stop active, simple dark mode when controls are active, using GRA controls as exp. long control (still on progress), panda not working for any other brands than volkswagen.
